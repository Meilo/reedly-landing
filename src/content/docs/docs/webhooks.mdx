...
{/* Adding a language is one more <TabItem>, nothing else. syncKey ties every
    Tabs block on the site to the same choice, so a reader who picks their
    language once keeps it everywhere. */}
<Tabs syncKey="lang">
...
Your language isn't here? Ask us — the primitive is HMAC-SHA256 over the raw body, hex-encoded, and every stack has it.
